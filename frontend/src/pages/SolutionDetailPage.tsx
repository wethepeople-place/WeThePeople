...
export default function SolutionDetailPage() {
  ...
  return <main className="min-h-screen bg-bg px-5 py-12 text-text-1"><article className="mx-auto max-w-4xl">
    ...
    <div className="mt-6 flex flex-wrap gap-3"><button disabled={!isAuthenticated || item.status !== 'published'} aria-pressed={item.current_user_choice === 'support'} onClick={() => vote('support')} className="rounded border border-emerald-500 px-4 py-2 disabled:opacity-50">Support · {item.vote_totals.support}</button><button disabled={!isAuthenticated || item.status !== 'published'} aria-pressed={item.current_user_choice === 'oppose'} onClick={() => vote('oppose')} className="rounded border border-rose-500 px-4 py-2 disabled:opacity-50">Oppose · {item.vote_totals.oppose}</button>{canEdit && <button className="rounded border border-accent px-4 py-2" onClick={() => setEditing((value) => !value)}>Revise</button>}</div>
