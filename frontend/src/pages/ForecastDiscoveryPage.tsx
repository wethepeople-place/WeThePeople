...
export default function ForecastDiscoveryPage() {
  ...
  return <main className="min-h-screen bg-[#eef1f5] pb-28 text-slate-950 md:pb-16">
    <header className="bg-[#153d7a] px-5 py-7 text-white"><div className="mx-auto max-w-5xl"><p className="text-xs font-black uppercase tracking-[.18em] text-blue-100">Civic Forecasts</p><h1 className="mt-1 text-3xl font-black sm:text-5xl">Open questions</h1><p className="mt-3 max-w-2xl leading-7 text-blue-50">Explore real civic questions already opened by participants. Forecasts use no money, prizes, payouts, or transferable points.</p></div></header>
    <div className="mx-auto max-w-5xl px-4 py-5 sm:px-6">
      ...
      {loading && <p className="mt-8 text-slate-600">Loading open civic questions…</p>}
      ...
      <section aria-label="Open forecasts" className="mt-5 grid gap-4 md:grid-cols-2">{markets.map((market) => {
        ...
        return <article key={market.id} className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
          <div className="flex items-center justify-between gap-3"><span className="inline-flex items-center gap-2 text-xs font-black uppercase tracking-widest text-[#214f78]">{market.market_type === 'bill' ? <Landmark className="h-4 w-4" /> : <Vote className="h-4 w-4" />}{market.market_type}</span><span className="inline-flex items-center gap-1 text-xs text-slate-500"><CalendarDays className="h-4 w-4" />Closes {closeLabel(market.closes_at)}</span></div>
          <h2 className="mt-3 text-xl font-black leading-7">{market.question}</h2>
          <div className="mt-4 space-y-2">{market.options.map((option) => <div key={option.key} className="rounded-xl bg-slate-50 px-3 py-2"><div className="flex justify-between gap-3 text-sm"><strong>{option.label}{option.party ? ` · ${option.party}` : ''}</strong><span>{visible ? `${option.share}%` : 'Private'}</span></div>{visible && <div className="mt-2 h-2 overflow-hidden rounded-full bg-slate-200"><div className="h-full rounded-full bg-[#dda91f]" style={{ width: `${option.share || 0}%` }} /></div>}</div>)}</div>
