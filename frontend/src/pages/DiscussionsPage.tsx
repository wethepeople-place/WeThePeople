...
export default function DiscussionsPage() {
  ...
  const submit = async (event: FormEvent) => {
    ...
    try {
      ...
      setBody(''); setVideoUrl(''); setSelectedIssue(issue); setSuggestedTitle(''); setNotice(`${result.message}. It will appear here after review.`);
      ...
      <div className="mt-7 grid items-start gap-7 lg:grid-cols-[minmax(0,1fr)_18rem]">
        <div className="min-w-0">
          <section ref={composer} id="composer" className="scroll-mt-20 rounded-2xl border border-border bg-surface p-5 sm:p-6" aria-labelledby="share-video-heading">
            ...
            <p className="mt-2 text-sm leading-6 text-text-2">Paste a Facebook, TikTok, Instagram, or YouTube link. We suggest the Agenda issue automatically, and every post is reviewed before appearing publicly.</p>
            {isAuthenticated ? <form className="mt-5 space-y-4" onSubmit={submit}>
              <label className="block text-sm font-semibold">Video link<input className="mt-2 min-h-11 w-full rounded-xl border border-border bg-bg px-4 py-3 text-text-1" type="url" required placeholder="Paste a Facebook, TikTok, Instagram, or YouTube link" value={videoUrl} onChange={(event) => setVideoUrl(event.target.value)} /></label>
              {videoUrl && !issue && <div aria-live="polite" className="rounded-xl border border-border bg-bg p-4">
                {suggestionState === 'loading' && <p className="text-sm text-text-2">Finding the best Agenda issue…</p>}
                ...
                {suggestionState === 'unmatched' && <p className="text-sm text-text-2">We could not confidently match this link. Choose the closest issue.</p>}
                {suggestionState === 'error' && <p className="text-sm text-text-2">Automatic matching is temporarily unavailable. Choose the closest issue.</p>}
              </div>}
              ...
              <label className="block text-sm font-semibold">Add your take <span className="font-normal text-text-3">(optional)</span><textarea className="mt-2 min-h-24 w-full rounded-xl border border-border bg-bg px-4 py-3 text-text-1" maxLength={10000} placeholder="What should people examine?" value={body} onChange={(event) => setBody(event.target.value)} /></label>
              ...
              <button disabled={submitting || suggestionState === 'loading' || !selectedIssue} className="min-h-11 rounded-full bg-accent px-5 py-3 font-bold text-white disabled:opacity-60">{submitting ? 'Submitting…' : 'Submit for review'}</button>
