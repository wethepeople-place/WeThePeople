  ...
  return <article className="overflow-hidden rounded-2xl border border-border bg-surface shadow-sm" aria-labelledby={`discussion-${item.id}`}>
    <div className="p-4 sm:p-5">
      ...
      <div className="mt-4 flex items-center justify-between border-t border-border pt-2" aria-label="Discussion actions">
        <Link className="inline-flex min-h-10 min-w-10 items-center justify-center gap-1 text-sm text-text-2 hover:text-accent-text" to={isAuthenticated ? `/discuss/${item.id}/reply` : `/login?next=${encodeURIComponent(`/discuss/${item.id}/reply`)}`} aria-label={isAuthenticated ? `Reply to discussion, ${item.reply_count} ${item.reply_count === 1 ? 'reply' : 'replies'}` : 'Sign in to reply'}><MessageCircle className="h-4 w-4" />{item.reply_count}</Link>
