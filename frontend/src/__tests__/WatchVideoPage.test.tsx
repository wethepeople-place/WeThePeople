...
describe('WatchVideoPage', () => {
  ...
  it('shows an automatically-fed community video with its proxied thumbnail', async () => {
    ...
    expect(screen.getByRole('button', { name: /Open comments for this video/ })).toBeTruthy()
    expect(screen.queryByRole('link', { name: /Open this community conversation/ })).toBeNull()
  ...
  it('opens the shared video conversation without leaving Watch', async () => {
    ...
    expect(screen.queryByRole('link', { name: "View this video's full conversation" })).toBeNull()
    expect(screen.queryByRole('link', { name: 'Open full discussion' })).toBeNull()
