...
it('performs a private election lookup and renders the official civic answer', async () => {
  vi.stubGlobal('fetch', vi.fn().mockImplementation((input: string) => Promise.resolve({
    ok: true,
    json: async () => input.endsWith('/elections') ? { items: [{ id: '9', name: 'Maryland General Election', election_day: '2026-11-03', division_id: 'ocd-division/country:us/state:md' }] } : {
...
it('explains that a ZIP code alone cannot identify a ballot without sending it', async () => {
  const fetchMock = vi.fn().mockResolvedValue({ ok: true, json: async () => ({ items: [{ id: '9', name: 'Maryland General Election', election_day: '2026-11-03', division_id: 'ocd-division/country:us/state:md' }] }) });
