...
# Minimal fallback: 3-digit zip prefix → state code (covers all 50 states + DC)
_FALLBACK_ZIP_STATE: Dict[str, str] = {
...
def verify_subscription(token: str, db: Session = Depends(get_db)):
...
def preview_digest(zip_code: str, db: Session = Depends(get_db)):
    """
    ...
    for member in members:
        ...
        # Anomalies for this member (table may not exist yet if detect_anomalies hasn't run)
        try:
            anomalies = (
                db.query(Anomaly)
                .filter(
